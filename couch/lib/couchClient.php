...
// 			return $this->_continuousChanges();
	...
	*/
	public function deleteDoc ($docID, $docRev ) {
		//if ( !is_object($doc) )	throw new InvalidArgumentException ("Document should be an object");
		if (is_null($docID) OR is_null($docRev) )    {
			throw new Exception("_id and _rev have to be specified");
			return FALSE;
		}
		$url = '/'.urlencode($this->dbname).'/'.urlencode($docID).'?rev='.urlencode($docRev);
